<script lang="ts">
  ...
  const headerClass = $derived(() => {
    let cls = 'tool-header flex items-center gap-2 px-3 py-2 ';
  ...
  const resultClass = $derived(() => {
    let cls = 'tool-result px-3 py-2 border-t border-[var(--border-color)] max-h-40 overflow-y-auto ';
...
<div class="tool-call-block border border-[var(--border-color)] rounded-lg overflow-hidden my-2 text-sm min-w-0 w-full">
  ...
  <div class={headerClass()}>
    {#if isExecuting}
      <div class="spinner animate-spin w-4 h-4 border-2 border-current border-t-transparent rounded-full"></div>
    ...
    <span class="tool-name font-mono font-medium text-[var(--text-primary)]">
      {call.function.name}
    </span>
  ...
  <!-- Arguments -->
  {#if args && args !== '{}'}
    <div class="tool-args px-3 py-2 bg-[var(--bg-tertiary)] border-t border-[var(--border-color)]">
      <pre class="text-xs overflow-x-auto text-[var(--text-secondary)] font-mono">{args}</pre>
    </div>
  {/if}
  ...
  {#if result}
    <div class={resultClass()}>
      <pre class="text-xs overflow-x-auto whitespace-pre-wrap text-[var(--text-secondary)]">{truncateContent(result.content)}</pre>
    </div>
  {/if}
</div>
